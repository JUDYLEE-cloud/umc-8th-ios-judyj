...
class LocationManager: NSObject {
    ...
    // MARK: - 지오코딩
    /// 주소를 좌표로 변환하는 비동기 메서드
    func geocode(address: String) async throws -> CLLocationCoordinate2D {
        let geocoder = CLGeocoder()
        let placemarks = try await geocoder.geocodeAddressString(address)
        
        guard let location = placemarks.first?.location else {
            throw NSError(domain: "LocationManager", code: 404, userInfo: [NSLocalizedDescriptionKey: "주소로부터 위치를 찾을 수 없습니다."])
        }
        
        return location.coordinate
    }

    /// 좌표를 주소로 변환하는 비동기 메서드
    func reverseGeocode(location: CLLocation) async throws -> String {
        let geocoder = CLGeocoder()
        let placemarks = try await geocoder.reverseGeocodeLocation(location)
        
        guard let placemark = placemarks.first else {
            throw NSError(domain: "LocationManager", code: 404, userInfo: [NSLocalizedDescriptionKey: "위치로부터 주소를 찾을 수 없습니다."])
        }
